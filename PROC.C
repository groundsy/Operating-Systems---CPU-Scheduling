 ...
 *
 * Changes:
 *   Feb 18, 2013     Modified balance_queues, sched, and enqueue. (Eric Grounds)	
 ...
 */
  ...
  struct proc *cp, *pp;				/* temporary proc pointers */
  ...
  /* If this is a user process, we have to determine
   * where in the USER_Q it should go. 
   * The position where it's inserted is determined
   * by the recent cpu time the process had.
   * Processes with a low recent cpu time should be inserted
   * before processes with a higher recent cpu time.
   * The process with the lowest cpu time after the last
   * recent cpu time calculation will be the head of the queue.
   */

  if (q == USER_Q) {
     cp = rdy_head[q];                   /* current process (cp) = rdy_head of queue.  */
     pp = NIL_PROC;                      /* previous process (pp) = NIL_PROC.          */

     if (cp == NIL_PROC) {               /* if there is no process ready in the queue, */
         rdy_head[q] = rdy_tail[q] = rp; /* Set rp to the head and tail of the queue.  */
         rp->p_nextready = NIL_PROC;     /* Mark the new end of rp                     */
     }
     else {				               /* Else, there are processes in  */
        while (cp != NIL_PROC &&	               /* queue,find where to insert rp.*/
     	    rp->p_rec_cpu_time > cp->p_rec_cpu_time) { /* While rp rec cpu time is      */
            pp = cp;				       /* greater than cp's, move down  */
            cp = cp->p_nextready;		       /* the list until we find the    */ 
        }				               /* position to insert rp at.     */

     	if (pp == NIL_PROC && cp != NIL_PROC) {        /* Add rp to head of queue.      */
            rp->p_nextready = cp;		       /* Chain head of queue.          */
            rdy_head[q] = rp;			       /* Set new queue head.           */
        }
     	else if (pp != NIL_PROC && cp != NIL_PROC) {   /* Insert rp between 2 processes */
            pp->p_nextready = rp;	               /* Chain rp to previous next.    */
            rp->p_nextready = cp;                      /* Chain cp to rp next.          */
        }
        else {
            rdy_tail[q]->p_nextready = rp;	       /* Add rp to tail of queue.      */
  ...
  /* If the processes being enqueued is not a user process,
   * enqueue it using the Minix 3 default.
   */
  else {  
 ...
 * User process priorities are not changed in this function, All user
 * processes are inserted into USER_Q (14). 
 */
  ...
  if (! time_left) {				/* quantum consumed ? */
      ...
      if (rp->p_priority < (IDLE_Q-1)) {
         if (!isuserp(rp)) {			/* If process is not a user process */
            rp->p_priority += 1;	        /* lower priority */
            if (rp->p_priority == USER_Q)       /* Avoid putting non-user processes */
               rp->p_priority -= 1;             /* in USER_Q.			    */
         }
      }
  }
...
#define P_BASE 60   /* Base used to calculate/decay recent CPU time */
