/* Changes:
 *   Feb 18, 2013  Modified the sched_dmp to display more information. (Eric Grounds)
 *                 Q_nr      = Queue number that the process is in (can be from 0 to 15)
 *                 Q_pos     = The position in the queue that the process is in (0,1,2,3, etc)
 *                 P_nr      = The process number
 *                 P_name    = The name of the process
 *                 P_pri     = The priority of a process (corresponds to what queue the process is in)
 *                 P_rec_cpu = The recent cpu time the process has had
 */
...
PUBLIC void sched_dmp()
{
  ...
  printf("Q_nr--Q_pos--P_nr--P_name--P_pri--P_rec_cpu\n");
  for (r=0;r<NR_SCHED_QUEUES; r++) {
      rp = rdy_head[r];
      if (!rp) continue;
    /*  printf("%2d: ", r);*/
      for (p=0; rp != NIL_PROC; p++) {
      	  printf("%3d   %3d   %3d   %8s  %2d    %5d",
      	        r, p, rp->p_nr, rp->p_name,
      	        rp->p_priority,
	        rp->p_rec_cpu_time);
      	  rp = rp->p_nextready;
    /*  while (rp != NIL_PROC) {
